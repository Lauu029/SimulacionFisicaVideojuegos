...
Proyectile::Proyectile(ShotType currentShotType, Vector3 pos, Vector3 dir)
{
	...
	case PISTOL:
		...
		setDamping(0.99);
		setRender(particleType::Sphere, 1.0,0.0,0.0,  { .75,.75,.75,1 });
		break;
	case ARTILLERY:
	...
	case FIREBALL:
		...
		setDamping(0.9f);
		setRender(particleType::Sphere, 1.5, 0.0, 0.0,  { .88,.34,0.13,1 });
		break;
	case LASER:
		setMass(0.1f); // almost no weight
		setVelocity(dir *400); // No gravity
		setAcceleration({ 0.0f, 0.0f, 0.0f });
		setDamping(0.99f);
		setRender(particleType::Sphere, 0.5, 0.0, 0.0, { 1,0,0,1 });
		break;
	}
	setPosition(pos);
